...
int getCorner(vector<vector<int>> board, int player){
  ...
  if(board[0][0]==opponent) opCorners++;
  if(board[7][0]==opponent) opCorners++;
  if(board[0][7]==opponent) opCorners++;
  if(board[7][7]==opponent) opCorners++;

  return 100 * (myCorners - opCorners) / (myCorners + opCorners + 1);
}

bool legalMove(vector<vector<int>> board, int player, int i, int j){
...
vector<pair<int, int>> getAllMoves(vector<vector<int>> board, int player){
  vector<pair<int, int>> result;
  for(int i = 0; i < 8; i++){
    for(int j = 0; j < 8; j++){
      if(legalMove(board, player, i, j)){
...
int getMobility(vector<vector<int>> board, int player){
  int oplayer = (player==1) ? 2 : 1;

  int myMoveCount = getAllMoves(board,player).size();
  int opMoveCount = getAllMoves(board,oplayer).size();

  return 100 * (myMoveCount - opMoveCount) / (myMoveCount + opMoveCount + 1);
}

int getPlayerDiscCount(vector<vector<int>> board, int player){
  int cnt = 0;
  for(int i = 0; i < 8; i++){
    for(int j = 0; j < 8; j++){
      if(board[i][j] == player){
...
int getDiscDiff(vector<vector<int>> board, int player){
   int oplayer = (player==1) ? 2 : 1;

   int mySC = getPlayerDiscCount(board,player);
   int opSC = getPlayerDiscCount(board,oplayer);

   return 100 * (mySC - opSC) / (mySC + opSC);
}

int getParity(vector<vector<int>> board){
  int rem = 64 - getPlayerDiscCount(board, 1) - getPlayerDiscCount(board, 2);
  return rem % 2 == 0 ? -1 : 1;
}

double heuristic(State& s, int player) {
    ...
    if(cnt < 20){
      return 1 * getCorner(board, player) + 0.05 * getMobility(board, player);
    }
    else if(cnt <= 58){
      return 1 * getCorner(board, player) + 0.02 * getMobility(board, player) + 0.01 * getDiscDiff(board, player) + 0.1 * getParity(board);
    }
    else{
      return 1 * getCorner(board, player) + 0.01 * getMobility(board, player) + 0.5 * getDiscDiff(board, player) + 0.5 * getParity(board);
    }
